...
<form method="POST" action="{{ $mode === 'create' ? route('users.store') : route('users.update', $user) }}" class="panel">
    ...
    <div class="form-grid">
        ...
        <div class="field"><label>الدور</label><select name="role"><option value="designer" @selected(old('role', $user->role)==='designer')>مصمم</option><option value="admin" @selected(old('role', $user->role)==='admin')>إدارة</option></select></div>
